...
vector<vector<float>> blur(vector<vector<float>> grid, float blurring)
{
	...
	vector<float> row1 = {cornerProb, adjacentProb, cornerProb};
	vector<float> row2 = {adjacentProb, centerProb, adjacentProb};
	vector<float> row3 = {cornerProb, adjacentProb, cornerProb};

	vector<vector<float>> window = {row1, row2, row3};
		...
		for (int j = 0; j < width; j++)
		{
			...
			for (int dx = -1; dx < 2; dx++)
			{
				for (int dy = -1; dy < 2; dy++)
				{
					...
					float newI = abs(i + dy) % height;
					float newJ = abs(j + dx) % width;
